...
type inferenceContribution struct {
	...
	Assumption     string
	Weight         float64
	ConfidenceOnly bool // true for reinforcement rules that boost confidence without setting new fields
}
...
func InferResistor(obs ObservedResistor) (InferenceResult, error) {
	...
	for _, rule := range rules {
		...
		// If rule did not modify state and is not a pure-confidence reinforcement, ignore it
		if !appliedEffectively && !contrib.ConfidenceOnly {
			continue
		}
